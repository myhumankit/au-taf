...
{
  uint16_t xs = BSP_LCD_GetXSize();
#if (defined(LCD_SOUTH) || defined(LCD_WEST))
  uint16_t ys = BSP_LCD_GetYSize();
#endif
  
  /* Write data value to all SDRAM memory */
  //  *(__IO uint32_t*) (LtdcHandler.LayerCfg[ActiveLayer].FBStartAdress + (4*(Ypos*BSP_LCD_GetXSize() + Xpos))) = RGB_Code;
#ifdef LCD_EAST  
  *(__IO uint16_t*) (LtdcHandler.LayerCfg[ActiveLayer].FBStartAdress + (2*(Xpos*BSP_LCD_GetXSize() + (xs-Ypos-1)))) = RGB_Code;
#else
  #ifdef LCD_SOUTH
  *(__IO uint16_t*) (LtdcHandler.LayerCfg[ActiveLayer].FBStartAdress + (2*((ys-Ypos-1)*BSP_LCD_GetXSize() + (xs-Xpos-1)))) = RGB_Code;
  #else
  #ifdef LCD_WEST
  *(__IO uint16_t*) (LtdcHandler.LayerCfg[ActiveLayer].FBStartAdress + (2*((ys-Xpos-1)*BSP_LCD_GetXSize() + (Ypos)))) = RGB_Code;
  #else
...
static void DrawChar(uint16_t Xpos, uint16_t Ypos, const uint8_t *c)
{
  ...
  for(i = 0; i < height; i++)
  {
    ...
    for (j = 0; j < width; j++)
    {
      ...
      {
#ifdef FONT_MAG2
        BSP_LCD_DrawPixel(Xpos + 2*j, Ypos+i, DrawProp[ActiveLayer].TextColor);
        BSP_LCD_DrawPixel(Xpos + 2*j, Ypos+i+1, DrawProp[ActiveLayer].TextColor);
        BSP_LCD_DrawPixel(Xpos + 2*j+1, Ypos+i, DrawProp[ActiveLayer].TextColor);
        BSP_LCD_DrawPixel(Xpos + 2*j+1, Ypos+i+1, DrawProp[ActiveLayer].TextColor);
#else
      ...
      {
#ifdef FONT_MAG2
        BSP_LCD_DrawPixel(Xpos + 2*j, Ypos+i, DrawProp[ActiveLayer].BackColor);
        BSP_LCD_DrawPixel(Xpos + 2*j, Ypos+i+1, DrawProp[ActiveLayer].BackColor);
        BSP_LCD_DrawPixel(Xpos + 2*j+1, Ypos+i, DrawProp[ActiveLayer].BackColor);
        BSP_LCD_DrawPixel(Xpos + 2*j+1, Ypos+i+1, DrawProp[ActiveLayer].BackColor);
#else
